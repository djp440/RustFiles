...
- 当前可用验证命令：`npm run check:all`、`npm run tauri:dev:test`、`cargo test --manifest-path src-tauri/Cargo.toml --test command_whitelist`、`cargo test --manifest-path src-tauri/Cargo.toml --test path_safety`。
- `scripts/run-tauri-dev-test.ps1` 已修复为自动结束 `tauri dev` 进程树，避免 smoke 测试后残留窗口或子进程。
- 下一实施任务：进入 `Task 1.2`，实现只读目录枚举和侧边栏 roots。
