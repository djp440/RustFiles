...
## 用户规则
- 使用wezterm-session-control skill（配合 WezTerm）来管理终端会话，并在WezTerm中运行测试/编译/安装依赖/服务端等需要长时间占用shell的命令，避免将会话阻塞
...
- 已完成 `Task 5.1`：Rust 最小任务状态机迁移边界（`TaskSummary`、`apply_task_transition`）、前端 `tasks` store、`TaskPanel` 摘要/展开态、browser-preview fallback 任务数据，以及 `task-panel` 单测与 Playwright browser-preview 验证。
- 已完成 `Task 5.2`：Rust 基础文件操作契约与系统集成（新建文件夹、重命名、删除到回收站、永久删除确认、默认应用打开、终端打开、属性）、前端 `InlineRename` / `DeleteConfirmDialog` / `PropertiesDialog` 接线，以及 Playwright `basic-file-ops` browser-preview 验证。
- 已完成 `Task 6.1`：Rust clipboard 模块（`clipboard.rs`）管理 pending operation（ClipOperation/ClipOpType/ClipOpStatus）、`tasks.rs` 添加 `execute_copy_items`/`execute_move_items`（多源支持、部分完成、测试根保护）、`commands.rs` 实现 `copy_items`/`move_items`/`create_clipboard_operation`/`paste_clipboard_operation`（含 guard_dangerous_operation 和 cut 分支 check_confirmation）、前端 `selection.ts` store（selectedPaths/clipboardOp/isCutPending），以及 `clipboard_ops`（17 测试）和 `clipboard-ui`（8 测试）单元测试。
- 已完成 `Task 6.2`：Rust drag 模块（`drag.rs`）管理 drag operation（DragOpType/DragOpStatus/DragOperation/DragManager）、`commands.rs` 实现 `create_drag_operation`/`drop_drag_operation`（跨卷检测、requested_type 修饰键切换）、前端三种视图的 draggable 支持和 HTML5 DnD API 集成、FileBrowser dragOver/drop 事件、TaskPanel progress 元素显示，以及 `drag_ops`（9 测试）和 `copy_move_progress`（5 测试）单元测试。
- 当前可用验证命令：`npm run check:all`、`npm run tauri:dev:test`、`npm run e2e -- e2e/navigation.spec.ts`、`npm run e2e -- e2e/view-sort-filter.spec.ts`、`npm run e2e -- e2e/glass-readability.spec.ts`、`npm run e2e -- e2e/performance.spec.ts`、`npm run e2e -- e2e/ui-priority.spec.ts`、`npm run e2e -- e2e/search.spec.ts`、`npm run e2e -- e2e/task-panel.spec.ts`、`npm run e2e -- e2e/basic-file-ops.spec.ts`、`npm run test -- src/test/interaction-reporting.test.ts`、`npm run test -- src/test/search-store.test.ts`、`npm run test -- src/test/task-panel.test.tsx`、`npm run test -- src/test/inline-rename.test.tsx`、`npm run test -- src/test/clipboard-ui.test.ts`、`cargo test --manifest-path src-tauri/Cargo.toml --test command_whitelist`、`cargo test --manifest-path src-tauri/Cargo.toml --test path_safety`、`cargo test --manifest-path src-tauri/Cargo.toml --test fs_listing`、`cargo test --manifest-path src-tauri/Cargo.toml --test sort_filter`、`cargo test --manifest-path src-tauri/Cargo.toml --test settings`、`cargo test --manifest-path src-tauri/Cargo.toml --test sidebar_roots`、`cargo test --manifest-path src-tauri/Cargo.toml --test large_directory`、`cargo test --manifest-path src-tauri/Cargo.toml --test scheduler`、`cargo test --manifest-path src-tauri/Cargo.toml --test search`、`cargo test --manifest-path src-tauri/Cargo.toml --test task_state_machine`、`cargo test --manifest-path src-tauri/Cargo.toml --test basic_file_ops`、`cargo test --manifest-path src-tauri/Cargo.toml --test system_integration`、`cargo test --manifest-path src-tauri/Cargo.toml --test clipboard_ops`、`cargo test --manifest-path src-tauri/Cargo.toml --test drag_ops`、`cargo test --manifest-path src-tauri/Cargo.toml --test copy_move_progress`。
...
- `e2e/task-panel.spec.ts` 同样依赖 `http://localhost:1420`；若 `TaskPanel` 同时渲染任务 message 和状态标签中的同一文案，Playwright 断言必须避免 strict mode 歧义，优先使用 `.first()` 或更窄作用域。
- `e2e/basic-file-ops.spec.ts` 当前是 browser-preview 证据：它验证前端新建/重命名/删除确认/属性弹窗/系统打开回退反馈的用户路径，但不等同于桌面全栈文件操作证据。若要验证真实系统打开、副作用写入和测试根保护，需要结合 `npm run tauri:dev:test`。
...
- clipboard 模块的 `paste_clipboard_operation` 命令必须包含 `guard_dangerous_operation`，因为 cut 操作涉及源文件删除（破坏性行为）。cut 分支还需 `check_confirmation`。
- `execute_move_items` 必须委托给 `execute_copy_items`，而不是内联复制逻辑。复制全部成功后逐项删除源，部分失败时保留源文件（架构要求：移动失败优先保证源不丢失）。
- `copy_items` 和 `move_items` 命令必须通过 `create_clipboard_operation` + `paste_clipboard_operation` 链路执行，确保经过完整安全校验链，不要直接调用 `clipboard::create_operation` + `tasks::execute_copy_items`。
- paste 操作后必须调用 `mark_pasted` 将操作标记为已粘贴，防止重复粘贴。
